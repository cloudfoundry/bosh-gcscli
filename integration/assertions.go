...
func NewAssertContext() AssertContext {
...
func (ctx *AssertContext) AddConfig(config *config.GCSCli) {
	ctx.Config = config
	ctx.ConfigPath = MakeConfigFile(config)
}

// AsReadOnly returns a copy of the AssertContext configured to be used
// soley for public, read-only operations.
//
// Note: The returned AssertContext is like any unique AssertContext in that
// Cleanup must be called to remove testing files from the filesystem.
func (ctx *AssertContext) AsReadOnly() *AssertContext {

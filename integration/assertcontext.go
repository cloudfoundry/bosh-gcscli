...
func AsDefaultCredentials(ctx *AssertContext) {
	...
	os.Setenv(GoogleAppCredentialsEnv, ctx.serviceAccountPath) //nolint:errcheck
...
func (ctx *AssertContext) Cleanup() {
	os.Remove(ctx.ConfigPath)  //nolint:errcheck
	os.Remove(ctx.ContentFile) //nolint:errcheck

	if ctx.serviceAccountPath != "" {
		os.Remove(ctx.serviceAccountPath) //nolint:errcheck
	}
	os.Unsetenv(GoogleAppCredentialsEnv) //nolint:errcheck
}
